...
class UIController {
    ...
    updateWordCount() {
        ...
        if (!this.currentProject || !this.currentProject.id) {
            ...
            if (!this.hasLoggedNoProject) {
                console.log('⚠️ No current project set for word count update');
    ...
    saveDraft() {
        ...
        try {
            ...
            projectManager.updateProject(this.currentProject.id, updateData);
            
            // Update current project reference
            this.currentProject = projectManager.getProject(this.currentProject.id);
    ...
    exportDocx() {
        console.log('📄 exportDocx() called');
        
        if (!this.currentProject || !this.elements.transcriptionEditor) {
            console.error('❌ Missing currentProject or transcriptionEditor for DOCX export');
            this.showErrorMessage('Cannot export DOCX: No project selected or editor not found');
            return;
        }
        
        console.log('✅ Exporting DOCX for project:', this.currentProject.name);
        
        try {
            const finalText = this.getRichTextContent(true); // Get plain text
            const projectName = UTILS.sanitizeFilename(this.currentProject.name);
            ...
            // Create a simple HTML-based approach for DOCX
            const docContent = this.createDocumentContent(finalText);
            
            // Create blob with the proper MIME type for Word documents
            ...
            link.download = `${projectName}_Final_Transcription.docx`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(link.href);
            
            console.log('📄 DOCX exported successfully:', link.download);
            this.showSuccessMessage('DOCX exported successfully!');
            
        } catch (error) {
            console.error('❌ Error exporting DOCX:', error);
            
            // Fallback to RTF format which is more compatible
...
${transcriptionText.replace(/\n/g, '\\par ')}
    ...
    // Export as RTF (fallback format that's Word-compatible)
    exportRtf() {
        console.log('📄 exportRtf() called as fallback');
        ...
        try {
            ...
            const rtfContent = this.createDocumentContent(finalText);
            ...
            const link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.download = `${projectName}_Final_Transcription.rtf`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(link.href);
            
            console.log('📄 RTF exported successfully:', link.download);
            this.showSuccessMessage('Document exported as RTF (Word-compatible format)!');
            
        } catch (error) {
            console.error('❌ Error exporting RTF:', error);
            this.showErrorMessage('Failed to export document: ' + error.message);
